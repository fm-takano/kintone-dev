...
(function() {
    ...
    function findExist(event) {
        ...
        // 社番変更時のみ新社番での検索も実施
        if(radioVal === '社番変更'){
            empCode = targetRow.value[radioVal+'_従業員番号_変更後'].value;
            ...
            // 結果件数が1以上（1以外はありえないけど。。）
            if (respData.records.length) {
                targetRow.value[radioVal+'_登録状況_従業員番号_新'].value = respData.records[0].従業員番号.value;
                targetRow.value[radioVal+'_登録状況_氏名_本人_新'].value = respData.records[0].氏名.value;
            }
            else {
                // 結果件数が0
                // 個人事業主登録状況テーブルの検索
                ...
                // 結果件数が1以上（1以外はありえないけど。。）
                if (respData.records.length) {
                    targetRow.value[radioVal+'_登録状況_従業員番号_新'].value = respData.records[0].個人支払先番号.value;
                    targetRow.value[radioVal+'_登録状況_氏名_本人_新'].value = respData.records[0].氏名.value;
                }
                else {
                    targetRow.value[radioVal+'_登録状況_従業員番号_新'].value = '';
                    targetRow.value[radioVal+'_登録状況_氏名_本人_新'].value = '';
                }
            }
        }
        return event;
    ...
    var eventList = [
        ...
        'app.record.create.change.社番変更_従業員番号_変更後',
        'app.record.edit.change.社番変更_従業員番号_変更後',
    ];
    ...
    function disabledField(event,radioVal){
        ...
        for (var i = 0; i < table.length; i++) {
            ...
            //従業員番号変更のみ
            if(radioVal === '社番変更'){
                rowData[radioVal+'_登録状況_従業員番号_新'].disabled = true;
                rowData[radioVal+'_登録状況_氏名_本人_新'].disabled = true;
            }
        }
        return event;
    }
